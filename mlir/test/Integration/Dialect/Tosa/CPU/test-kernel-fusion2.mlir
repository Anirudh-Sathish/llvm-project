...
func.func @main() {
  ...
  %d = tosa.add %c, %b : (tensor<2x3xf32>, tensor<2x3xf32>) -> tensor<2x3xf32>
  
  %c_unranked = tensor.cast %c : tensor<2x3xf32> to tensor<*xf32>
  %d_unranked = tensor.cast %d : tensor<2x3xf32> to tensor<*xf32>
...
// CHECK: Unranked Memref base@ = {{.*}} rank = 2 offset = 0 sizes = [2, 3] strides = [3, 1] data =
// CHECK-NEXT: [
// CHECK-SAME: [7,   7,   7]
// CHECK-NEXT: [7,   7,   7]
// CHECK-SAME: ]
